...
export async function listUnifiedBookingsForUser(userId: string): Promise<UnifiedBookingRecord[]> {
  const [hotelAndTourBookings, rentalBookings, activityBookings] = await Promise.all([
  ...
  const baseBookings: UnifiedBookingRecord[] = hotelAndTourBookings.map((booking) => {
    const isTour = Boolean(booking.tourId)
    ...
    return {
      ...
      title: booking.Hotel?.title ?? "Hotel booking",
      subtitle: "Stay reservation",
      location: booking.Hotel?.city ?? null,
      href: booking.Hotel?.slug ? `/hotels/${booking.Hotel.slug}` : "/hotels",
      paymentStatus: booking.Payment?.status ?? null,
    }
  })
